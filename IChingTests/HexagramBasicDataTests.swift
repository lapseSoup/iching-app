...
final class HexagramBasicDataTests: XCTestCase {
    ...
    func testAllHaveNonEmptyNames() {
        for hex in HexagramBasicInfo.all {
            XCTAssertFalse(hex.name.isEmpty, "Hexagram \(hex.id) has empty name")
    ...
    // MARK: - Consistency with HexagramLibrary
    ...
    func testBasicDataMatchesLibrary() {
        for basic in HexagramBasicInfo.all {
            ...
            XCTAssertEqual(basic.name, full.englishName, "Name mismatch for hexagram \(basic.id)")
